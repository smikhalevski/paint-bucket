...
import { convertLABToXYZ, convertXYZToLAB } from '../../../main/color-model/lab/index.js';

describe('convertXYZToLAB', () => {
  test('converts black XYZ to LAB', () => {
    expect(convertXYZToLAB([0, 0, 0, 1], [0, 0, 0, 1])).toStrictEqual([0, 0.5, 0.5, 1]);
  });

  test('converts white XYZ to LAB', () => {
    expect(convertXYZToLAB([0.95, 1, 1, 1], [0, 0, 0, 1])).toStrictEqual([
      0.9999999999999999, 0.5016600970369557, 0.523224638835883, 1,
    ]);
  });

  test('converts XYZ to LAB', () => {
    expect(convertXYZToLAB([0.25, 0.4, 0.15, 1], [0, 0, 0, 1])).toStrictEqual([
  ...
  test('examples from Matlab', () => {
    ...
    expect(convertXYZToLAB([0.25, 0.4, 0.1, 1], [0, 0, 0, 1])).toStrictEqual([
...
describe('convertLABToXYZ', () => {
  test('converts LAB to XYZ', () => {
    ...
    expect(convertLABToXYZ(lab, [0, 0, 0, 1])).toStrictEqual([0.25, 0.39999999999999997, 0.15000000000000005, 1]);
  });
});
