...
export function rgbToXyz(rgb: Rgb, xyz: Xyz, white = whitepoint[2].D65): Xyz {
  ...
  xyz[0] = clamp01(r * 0.412390799265950 + g * 0.35758433938387 + b * 0.180480788401830);
  xyz[1] = clamp01(r * 0.212639005871510 + g * 0.71516867876775 + b * 0.072192315360733);
  xyz[2] = clamp01(r * 0.019330818715591 + g * 0.11919477979462 + b * 0.950532152249660);
  xyz[3] = rgb[3];
...
export function xyzToRgb(xyz: Xyz, rgb: Rgb, white = whitepoint[2].D65): Rgb {
  ...
  rgb[0] = clamp01(rotateXyzToRgb(r) * white[0]);
  rgb[1] = clamp01(rotateXyzToRgb(g) * white[1]);
  rgb[2] = clamp01(rotateXyzToRgb(b) * white[2]);
  rgb[3] = xyz[3];
