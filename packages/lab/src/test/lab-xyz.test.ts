...
describe('xyzToLab', () => {
  test('converts black XYZ to LAB', () => {
    expect(xyzToLab([0, 0, 0, 1], [0, 0, 0, 1])).toEqual([0, 0.5, 0.5, 1]);
  });
  ...
  test('converts XYZ to LAB', () => {
    expect(xyzToLab([0.25, 0.4, 0.15, 1], [0, 0, 0, 1])).toEqual([
  ...
  test('examples from Matlab', () => {
    ...
    expect(xyzToLab([0.25, 0.4, 0.1, 1], [0, 0, 0, 1])).toEqual([
...
describe('labToXyz', () => {
  test('converts LAB to XYZ', () => {
    const lab = xyzToLab([0.25, 0.4, 0.15, 1], [0, 0, 0, 1]);

    expect(labToXyz(lab, [0, 0, 0, 1])).toEqual([0.25, 0.39999999999999997, 0.15000000000000005, 1]);
  });
});
